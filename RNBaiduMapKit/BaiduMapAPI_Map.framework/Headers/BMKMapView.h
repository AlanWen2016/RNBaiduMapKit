...
typedef enum : NSInteger {
    BMKMapParticleEffectUnknow = -1, // 未知，关闭粒子效果时传入该值将关闭所有粒子效果
    BMKMapParticleEffectSnow = 0,        // 雪
    BMKMapParticleEffectRainStorm = 4,   // 雨
    BMKMapParticleEffectSmog = 5,        // 雾霾
    BMKMapParticleEffectSandStorm = 7,   // 沙尘暴
    BMKMapParticleEffectFireworks = 8,   // 烟花
    BMKMapParticleEffectFlower = 9      // 花瓣
...
typedef enum {
    kBMKLayerSDKOverlay,    /// SDK覆盖物图层
    kBMKLayerSDKHeatMap,    /// SDK热力图图层
    kBMKLayerSDKHexagonHeatMap,    /// SDK蜂窝热力图图层
    kBMKLayerNewSDKDefaultOverlay   /// 新版SDK覆盖物默认图层
} BMKLayerType;
...
/// 自定义底图加载时的背景网格图片，仅支持png格式 since 6.5.9
/// 默认为背景网格图片
...
/// V6.5.9版本新增
/// 设置地图覆盖物是否使用新版逻辑绘制，默认为YES，应用启动期间仅可设置一次
 ...
 *@param animated 是否启动动画
 */
- (void)showAnnotations:(NSArray<id <BMKAnnotation>> *)annotations padding:(UIEdgeInsets)padding animated:(BOOL)animated;
...
@interface BMKMapView (HexagonHeatMapAPI)
/// 是否显示蜂窝热力图，默认YES，添加热力图后生效，since 6.6.0
@property (nonatomic, assign) BOOL showHexagonHeatMap;
/// 添加蜂窝热力图，since 6.6.0
...
/// @param layer 其中一个图层
/// @param otherLayer 另一个图层
/// @return 成功返回YES，否则返回NO
- (BOOL)switchLayerOrder:(BMKLayerType)layer otherLayer:(BMKLayerType)otherLayer;

/// 地图是否展示地形图层默认NO，since 6.5.9
@property (nonatomic, assign) BOOL showDEMLayer;
...
@interface BMKMapView (ParticleEffect)
/**
 显示粒子效果，since 6.5.7
...
/**
 ...
 @param option 自定义参数
 */
- (BOOL)customParticleEffect:(BMKMapParticleEffect)effect option:(BMKParticleEffectOption *)option;
@end
